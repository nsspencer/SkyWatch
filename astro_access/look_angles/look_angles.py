...
def calculate_reference_frame(x:np.ndarray, 
                              y:np.ndarray,
                              z:np.ndarray,
                              v_x:np.ndarray,
                              v_y:np.ndarray,
                              ...
                              abs_tolerance:float=1.e-9) -> np.ndarray:
    """
...
def get_look_angles_to(observer_pos: np.ndarray, target_pos: np.ndarray, observer_frame: Rotation) -> tuple:
    ...
    """
    assert observer_pos.ndim == target_pos.ndim, "Dimensions of observer and target positions must be equivalent."
    ...
    component_along_z = np.sum(observer_to_target * z_axis, axis=1)[:, np.newaxis] * z_axis
    observer_to_target_projected = observer_to_target - component_along_z
    
    # calculate the azimuth from the x axis
    azimuth = _counterclockwise_angle_between(x_axis, observer_to_target_projected[:, :2].value)
